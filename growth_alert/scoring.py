...
def risk_flags(record: dict, cfg: dict) -> list[str]:
    ...
    if not finite(age) or age < 0 or age > cfg["filters"]["max_statement_age_days"]:
        flags.append("STALE_OR_MISSING_STATEMENT")
    ...
    if finite(record.get("eps_revision_30d")):
...
def exclusions(record: dict, cfg: dict) -> list[str]:
    f = cfg["filters"]
    reasons = []
    for field, minimum in (("previous_close", f["min_price_usd"]),
                           ("market_cap", f["min_market_cap_usd"]),
                           ("dollar_volume_20d", f["min_dollar_volume_20d"]),
                           ("revenue_yoy", f["min_revenue_yoy"])):
        if not finite(record.get(field)) or record[field] < minimum:
            reasons.append(field.upper() + "_FILTER")
    if record.get("quote_currency") != "USD":
        reasons.append("QUOTE_CURRENCY_NOT_USD")
    if not record.get("daily_price_valid"):
        reasons.append("DAILY_PRICE_NOT_CURRENT")
    age = record.get("statement_age_days")
    if not finite(age) or not 0 <= age <= f["max_statement_age_days"]:
        reasons.append("STATEMENT_NOT_CURRENT")
    if record.get("sector") in f["excluded_sectors"]:
        reasons.append("EXCLUDED_SECTOR")
    if not any(finite(record.get(x)) for x in FEATURES["quality"]):
        reasons.append("NO_QUALITY_FEATURES")
    if finite(record.get("cash_runway_quarters")) and record["cash_runway_quarters"] < 4:
...
def percentile(value: float, pool: pd.Series) -> float:
    ...
    if len(values) < 2 or not finite(value):
        return 50.0
    # Average tied ranks; identical values yield 50, not 100.
...
def score_records(records: list[dict], cfg: dict) -> list[dict]:
    ...
    # Growth-eligible peers only. Speculative watch names never enter this pool.
    ...
    for item in all_records:
            ...
            for feature, internal_weight in features.items():
                ...
                if not peers.empty and feature in peers and finite(item.get(feature)) and not item["exclusions"]:
                    ...
                    if len(same_sector) >= min_peers:
                        ...
                        valid = pd.DataFrame()  # No cross-sector cheapness comparison.
                    elif len(valid) >= min_peers:
                        scope = "growth_universe_fallback"
                    else:
                        valid = pd.DataFrame()
                usable = not valid.empty
                subscore = percentile(item[feature], valid[feature]) if usable else 50.0
        ...
        if not item["eligible"]:
            item["score"] = None  # Do not present excluded names as real quant ranks.
